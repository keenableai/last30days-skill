...
| Web search | none required (Keenable is keyless); optional `BRAVE_API_KEY`, `EXA_API_KEY`, `SERPER_API_KEY`, `PARALLEL_API_KEY`, or `KEENABLE_API_KEY` | `--auto-resolve` and Step 2 supplements | Keenable works with no key; native WebSearch on Claude Code / Codex / Gemini works as a fallback |
...
4. **Keenable (keyless default)** - a real web-search API that works with **no key** (`--web-backend=keenable`, or auto-selected when no paid key is set and there is no host search). Optional `KEENABLE_API_KEY` only lifts the rate limit; it is never required. Preferred over the DuckDuckGo/SearXNG floor because it is a proper search index, not scraped HTML.
5. **Keyless engine floor** - zero-key web search (DuckDuckGo, plus an optional SearXNG instance) and zero-key page fetch (Jina Reader). Still reachable via `--web-backend=keyless`; auto-selection now prefers Keenable ahead of it. Runs only when the agent session has **no** host web search.
